 ...
 * daemon-as-agent phrasing and Apple-associated glass product copy. The
 * daemon is a "daemon" / "host daemon" / "turbopaneld", never an "agent" —
 * that word is reserved for coding-agent tooling (`AGENTS.md`,
 * `.agents/skills`) and unrelated third-party terms (HTTP `User-Agent`, npm
 * package names). Shell chrome is "frosted chrome".
 *
...
/** This checker's own phrase list, plus skill packs, must not be scanned. */
function isSkippedPath(rel) {
  return rel === SELF || /(^|\/)\.agents\/skills(\/|$)/.test(rel)
}

function isSkippedDir(name, rel) {
  return SKIP_DIR_NAMES.has(name) || isSkippedPath(rel)
}

function isSkippedFile(name, rel) {
  return SKIP_FILENAMES.has(name) || isSkippedPath(rel)
}
...
function* walk(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      yield* walk(abs)
    } else if (entry.isFile() && !isSkippedFile(entry.name, rel)) {
      yield abs
    }
  }
}
