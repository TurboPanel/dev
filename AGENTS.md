...
3. **Bootstrap / converge** → the console uses `resolveDevEnvStartupPlan` (`src/lib/dev-env-readiness.ts`) on launch: **auto-bootstraps** (daemon install → systemd unit → converge) when prerequisites are missing (`handleDaemonInstallDone` then calls `startDevEnvConverge("if-needed")`); **auto-converges** (`if-needed`, skip-when-unchanged) when the host is already installed. Set `TURBOPANEL_CONSOLE_NO_AUTO_CONVERGE` (any non-empty value) to skip auto-converge on launch — bootstrap is unaffected; Developer → **Converge / re-converge** (`start-dev-env`, force mode) still works. Daemon bootstraps as the dev user, writes `/etc/turbopanel/daemon.env`, runs the `dev/orchestration` overlay (runtimes into `/opt/turbopanel/vendor`, systemd units + Docker (postgres/redis/rabbitmq/mailpit) as the dev user, mutable data under FHS trees dev-user-owned; no `tp` / `tpctrl` / `tpcache` accounts created).
