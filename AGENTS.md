...
| `./scripts/sync.sh` | Push `turbopanel/src/db/schema.ts` → live Postgres (drizzle-kit push; Deno dev convenience). |
| `./scripts/introspect.sh` | Pull live Postgres → `turbopanel/src/db/schema.ts` (drizzle-kit introspect). |
...
| promote → rc/release | `promote.yml` (notes-only) | `promote.yml` → `gh-promote.yml` here: **signature + sha256/size of the tested build, re-signed manifest, same bytes** | no new code enters after publish |

## Release promotion (`gh-promote.yml`, `gh-release.yml`, `gh-promote-finalize.yml`)

The one-click promotion every repo runs (Testing Checklist `release` section, rel-s19..s21) lives here as three reusable workflows; each sibling's `.github/workflows/promote.yml` (`workflow_dispatch`, inputs `to` = `rc|release`, `source`) chains them as three `uses:` jobs pinned to one `TurboPanel/dev@<sha>` and passes that same sha as `dev-ref` (a reusable workflow cannot call another at a parameterised ref, and it runs on the caller's runner with nothing checked out — `dev-ref` is how it finds `scripts/promote/`). Order across repos: turbopaneld → turbopanel → ui → website → dev. dev's own `promote.yml` is notes-only and calls them locally (`./…`).

- **Version model** (decision D8, Option A): a canary is `<base>-canary.<yyyymmdd-hhmmss-sha7>`; `to=rc` makes `<base>-rc.1` (always `.1` — a bad rc is fixed on trunk with a changeset and becomes the next number, never rc.2); `to=release` makes `<base>`. Trunk at 0.1.1 with no `.changeset/` (the first release through this pipeline) promotes `0.1.1-canary.<id>` → `0.1.1-rc.1` → `0.1.1`.
- **`gh-promote.yml` (prepare, `environment: release`)** — inputs `repo-kind` (`daemon|instance|ui|notes-only`), `to`, `source` (rc: canary build id, canary version or `manifest-<version>.json`; notes-only: a commit-ish; release: the rc tag), `dev-ref`, `signer-ref` (turbopaneld commit with `scripts/sign-manifest.ts` + the key pin); secrets `RELEASE_SIGNING_KEY`, `RELEASE_APP_ID`, `RELEASE_APP_PRIVATE_KEY`; outputs `version`, `tag`, `commit`, `source-version`, `branch`, `assets-artifact-name`, `manifest-filename`, `release-notes`. It downloads the source build (rc: the assets the canary `manifest-<version>.json` copy names; release: the rc release), verifies the manifest's Ed25519 signature with the signer checkout's own `verifyManifestSignature` (`scripts/promote/verify-signature.ts`) and every asset's sha256/size, refuses an rc while `.changeset/*.md` are pending at the source commit (no `.changeset/` → notice + skip), renames the assets to the target version, rewrites the manifest (version, channel, tag-pinned urls; commit/buildId/builtAt/sha256 untouched; signature dropped), signs it with `sign-manifest.ts`, creates the tag at the source commit (idempotent — an existing tag on another commit is a burned version and fails), and uploads the assets. `to=release` refuses to start without the App secrets: bare `v<base>` tags and `staging`/`live` pushes need the **TurboPanel Release** GitHub App as ruleset bypass actor; `v<base>-rc.1` is created with `GITHUB_TOKEN` (fires no workflows). Every falsifiable check runs before the tag step (`src/lib/promote.test.ts` asserts the order).
...
- **`gh-promote-finalize.yml` (`environment: release`)** — release: `gh release edit --prerelease=false --latest`, then the signed release `manifest.json` is uploaded as the rolling `rc` pointer (consumers do not enforce `manifest.channel`; no second signing). Both hops: fast-forward `staging` (rc) / `live` (release) to the tag commit; a diverged branch gets a `--no-ff` merge commit (never squash, never force) and the run summary says so; without the App the push step fails with the exact `git push origin <sha>:refs/heads/<branch>` to run by hand.
- **Pure logic** is `scripts/promote/lib.mjs` (+ `lib.d.mts`, `cli.mjs`), tested by `src/lib/promote.test.ts`, which also pins the workflow shapes. `release.yml`'s tag-push path skips `[bot]` actors so an App-created tag does not race the promotion with a from-source rebuild.
- **Owner-side setup**: a `release` environment (required reviewer = owner, prevent-self-review off) in every calling repo holding `RELEASE_SIGNING_KEY` (or keep it repo-level) and, once the App exists, `RELEASE_APP_ID` / `RELEASE_APP_PRIVATE_KEY`; the App as bypass actor on the tag ruleset and `staging & live: review and CI` (`RELEASE_APP_BYPASS` in `scripts/enable-trunk-branch-protection.sh`). Two approvals per promotion (prepare, finalize).
