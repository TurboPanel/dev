...
├── console               # ensure Node+Deno, pnpm install, launch the TUI via vite-node
...
/usr/local/bin/deno        # pinned Deno (installed by ./console) — used by daemon/instance, not this repo
/opt/turbopanel/
├── platform/             # daemon and other platform repos (installed by daemon via Ansible)
└── runtimes/             # uv/python/ansible (daemon)
```

Node is a pinned `nodejs.org` tarball installed into `/usr/local`. Deno is a pinned GitHub release installed to `/usr/local/bin/deno`. pnpm is provisioned via Corepack and pinned by the `packageManager` field in `package.json`.
...
- Do not reintroduce `deno.json`/`deno.lock` to this repo or use Deno to run the console — this repo is Node/pnpm/Vite. (Deno stays installed for daemon/instance.)
