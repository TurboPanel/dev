...
**Bootstrap URL:** `dev.turbopanel.sh/develop.sh` (advertised one-liner; legacy alias `trbp.nl/develop.sh` redirects here) → `scripts/develop.sh` on the `trunk` branch of [turbopanel/dev](https://github.com/turbopanel/dev). When piped (`curl … | sh`), `$0` is `sh` so local `scripts/lib/` is not on disk yet — the script downloads those libs from `raw.githubusercontent.com/turbopanel/dev` (override with `TURBOPANEL_DEV_LIB_BASE`) before clone.
...
| Script | Purpose |
|--------|---------|
| `curl -fsSL dev.turbopanel.sh/develop.sh \| sh` | Clone/update `~/dev` via SSH, then launch the TUI. |
