...
provisioned interactively). Ports `8443` / `8880` / `8088` / `19820` forward
to the host on `0.0.0.0` (LAN-reachable, not localhost-only). Drizzle Studio
`4983` is loopback-only on both sides of the forward (`127.0.0.1` host and
guest): its API is unauthenticated, and the hosted HTTPS UI must use
`?host=localhost` rather than a private hostname such as `studio.lan`.
...
├── Vagrantfile           # UTM guest: mounts + port forwards + light provision
...
├── scripts/develop.sh    # clone/update + exec ./console (bare Debian)
├── scripts/vagrant-up.sh # macOS: vagrant up then ssh into ./console
...
3. **Bootstrap / converge** → the console uses `resolveDevEnvStartupPlan` (`src/lib/dev-env-readiness.ts`) on launch: **auto-bootstraps** (daemon install → systemd unit → converge) when prerequisites are missing (`handleDaemonInstallDone` then calls `startDevEnvConverge("if-needed")`); **auto-converges** (`if-needed`, skip-when-unchanged) when the host is already installed. Set `TURBOPANEL_CONSOLE_NO_AUTO_CONVERGE` (any non-empty value) to skip auto-converge on launch — bootstrap is unaffected; Developer → **Converge / re-converge** (`start-dev-env`, force mode) still works. Daemon bootstrap (`installDaemon` in `src/lib/platform-install.ts`) **uses an existing usable checkout** when `~/daemon` already has `main.ts` or `orchestration/ansible.cfg` (Vagrant VirtFS mounts and pre-cloned siblings — no guest-side clone/pull; Git may also refuse mounted trees via `safe.directory`); it only **clones** when that path is missing (bare-metal first run). Bootstrap then writes `/etc/turbopanel/daemon.env`, runs the `dev/orchestration` overlay (runtimes into `/opt/turbopanel/vendor`, systemd units + Docker (postgres/redis/rabbitmq/mailpit) as the dev user, mutable data under FHS trees dev-user-owned; no `tp` / `tpctrl` / `tpcache` accounts created).
...
| Script | Purpose |
|--------|---------|
| `./scripts/vagrant-up.sh` | **macOS:** `vagrant up --provider=utm`, then SSH into guest `./console`. |
| `curl -fsSL dev.turbopanel.sh \| sh` | **Bare Debian:** clone/update `~/dev` via SSH, then launch the TUI. |
...
```bash
# siblings: …/turbopanel/{dev,daemon,instance,ui,website}
cd …/turbopanel/dev
./scripts/vagrant-up.sh
```

**Typical flow (bare Debian):**
...
- **Dev environment readiness** — `src/lib/dev-env-readiness.ts` exports `resolveDevEnvStartupPlan`, which probes whether the console should auto-bootstrap, auto-converge (`if-needed`), or sit idle (`TURBOPANEL_CONSOLE_NO_AUTO_CONVERGE`). Wired from `initialAutoInstallState()` / `useConsoleApp`: bootstrap when checkout/Deno/ansible/unit are missing; otherwise auto-converge on launch unless the opt-out is set. Fast-skip of already-converged hosts is server-side (`instance-dev-install --if-needed`).
- **`instance-dev-install --if-needed`** — `installDevEnvironment(..., mode)` defaults to `"force"` for legacy callers (reset/provisioner) and for Developer → **Converge / re-converge** (`start-dev-env`). Uses `"if-needed"` for the daemon-install-finished chain (`handleDaemonInstallDone` → `startDevEnvConverge("if-needed")`) and for auto-converge on launch. Force mode sets `TURBOPANEL_FORCE_CONVERGE=1` via `orchestrationEnv()` so the daemon playbook always runs; `if-needed` lets the daemon skip when the converge stamp matches (see `../daemon/AGENTS.md`).
