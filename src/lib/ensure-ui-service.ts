...
export async function ensureUiServiceRuntimeDirs(): Promise<void> {
  ...
  const script = [
    ...
    `install -d -m 2770 -o ${INSTANCE_USER} -g ${TURBOPANEL_GROUP} "$cache"`,
    `find "$cache" -xdev -exec chown ${INSTANCE_USER}:${TURBOPANEL_GROUP} {} + 2>/dev/null || true`,
    `find "$cache" -type d -exec chmod 2770 {} + 2>/dev/null || true`,
