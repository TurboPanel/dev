...
| `lan-aliases` | `Vagrantfile` `dev-forward-hosts` provisioner | Extra DNS names (one per line, `#` comments) added to `/etc/turbopanel/dev-forward-hosts` beside the host's LAN addresses and FQDN. `TURBOPANEL_DEV_LAN_ALIASES` is the same list when it is set in the environment. Converge after the file changes so `instance-certs` puts the names on the Platform CA leaf. Add Server refuses a `:8443` name the leaf does not cover. |
